...
def load_clean_options(ticker="SPY", maturity = "2025-12-19"):
    ...
    calls = calls[["type", "contractSymbol", "strike", "lastPrice", "bid", "ask",
                   "volume", "openInterest", "impliedVolatility", "lastTradeDate", "inTheMoney"]]

    #Filter out options with non-positive bid or ask prices
    calls = calls[(calls["bid"] > 0) & (calls["ask"] > 0)].copy()

    #Compute the mid price (average of bid and ask)
    ...
    #Separate calls into in-the-money (ITM) and out-of-the-money (OTM)
    calls_itm = calls[calls["strike"] < S0].copy()
    calls_otm = calls[calls["strike"] > S0].copy()

    return  calls, calls_itm, calls_otm, T, S0

#Save cleaned call option data for SPY (full set, ITM, and OTM) along with time to maturity (T) and spot price (S0).
